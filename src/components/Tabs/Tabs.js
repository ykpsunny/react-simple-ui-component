...
function Tabs({ children, tabPosition, tabList, defaultActiveKey, onChange }) {
	const clas = classnames("simple-tabs-wrapper", tabPosition),
	...
	return (
		<div className={clas}>
			<ul className="simple-tab-title">{renderTabList(tabList)}</ul>
			<div className="simple-tab-content">{children}</div>
		</div>
	);
}
...
Tabs.propTypes = {
	...
	tabList: propTypes.arrayOf(
		propTypes.shape({
			tabKey: propTypes.oneOfType([propTypes.string, propTypes.number]).isRequired, // tab 切换唯一标识
