...
let commonClass = classnames("simple-pagination-item");

function Pagination({ current, pageSize, total, disibled, hideButton, onChange }) {
		...
		return (
			<li
				...
				className={classnames(commonClass, {
					"simple-pagination-item-disibled": prev || disibled
				})}
	...
	function renderNext() {
		return (
			<li
				key="right"
				className={classnames(commonClass, {
					"simple-pagination-item-disibled": next || disibled
				})}
		...
		i => {
			const isActive = i === stateCurrent,
				clas = classnames(commonClass, {
					...
					"simple-pagination-item-disibled": disibled
				});
			return (
